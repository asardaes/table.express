...
We can use the `.COL` pronoun in `transmute_sd`'s expression for `.SDcols`,
which can be functionally equivalent to `dplyr`'s `transmute_if`:

```{r transmute-if}
# transmute columns with only whole numbers
DT %>%
  start_expr %>%
...
However, this specific case is eager,
so explicit chaining will be required if multiple `transmute_sd` calls want to use `.COL`:

```{r multiple-transmute-sd}
DT %>%
  start_expr %>%
  transmute_sd(as.integer, .SDcols = all(.COL %% 1 == 0)) %>%
  chain %>%
  transmute_sd(sum, .SDcols = max(.COL) < 4L) %>%
  end_expr
```
