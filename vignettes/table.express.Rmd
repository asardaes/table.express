...
(with caveats, see [explicity chaining](#explicit-chaining)):
...
The `filter_on` verb can be used to build an expression that specifies the `on` argument of the query.
...
Some verbs use the captured `data.table` eagerly during expression building,
like `filter_sd` and `mutate_sd` when `.SDcols` is a `tidyselect` call.
