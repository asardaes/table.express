...
The `mutate` verb build an expression with `:=` in order to perform assignment by reference by default.
...
Moreover, `mutate_sd` supports a special `.COL` pronoun that symbolizes the column that should be modified,
