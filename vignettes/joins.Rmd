...
Almost all joins betwen 2 `data.table`s use a notation where one of them is used as `i` in a frame applied to the other,
...
This vignette will describe the notation to apply these joins with verbs defined `table.express`,
...
The `semi_join` verb is semi-eager in the sense that it starts a new expression chain automatically.
This is because it requires special internal name handling when some columns of both `data.table`s have the same name,
but are not part of the `on` expression:
...
As shown, the `left_join` case renamed `A`'s `v` to `i.v`.
A `semi_join` is essentially a `left_join` with `nomatch = NULL`,
`mult = "first"`,
and where only the left-hand side columns are selected afterwards,
hence the special handling required.

Finally, `full_join` is basically a wrapper for `merge` specifying `all = TRUE`,
so it's only useful when more expressions are to be added after the join:
...
so any subsequent verbs that specify `j`, `by`, or `keyby` would not trigger [automatic chaining](https://asardaes.github.io/table.express/articles/table.express.html#automatic-expression-chaining).
...
But would require explicit chaining for expressions that should be applied to the joined table:
...
so this won't work (not evaluated here):

```{r invalid-mutate-join, eval=FALSE}
B %>%
    start_expr %>%
...
This is because the column order in the expressions changed,
but we could understand the rolling expressions as:
...
Due to some of the processing performed by `left_join` during parsing,
different verbs are defined:
`lne_join` and `rne_join` for left/right non-equi joins.

One particularity of `lne_join` is that it only accepts comparison operators,
so using single `=` would be incorrect.
If the values of two columns should be equal and the columns have the same name,
then writing the column name is the same as using `==`.
Other than that,
their expectations are the same as what was explained for rolling joins:
the order of columns should correspond to the order in which the tables enter the expression,
and the name of the column that's part of the comparison could change.
